...
END
   ...
   To use the demoSp procedure with Connector/J, follow these steps:
    1. Prepare the callable statement by using Connection.prepareCall() .
       Notice that you have to use JDBC escape syntax, and that the parentheses
       surrounding the parameter placeholders are not optional:
...
1.1.4. Retrieving AUTO_INCREMENT Column Values
   ...
   Before version 3.0 of the JDBC API, there was no standard way of retrieving
   key  values from databases that supported 'auto increment' or identity
   columns. With older JDBC drivers for MySQL, you could always use a MySQL-
   specific method on the Statement interface, or issue the query 'SELECT
   LAST_INSERT_ID()'  after  issuing  an  'INSERT' to a table that had an
   AUTO_INCREMENT key. Using the MySQL-specific method call isn't portable, and
   issuing a 'SELECT' to get the AUTO_INCREMENT key's value requires another
   ...
   AUTO_INCREMENT values. First, we demonstrate the use of the new JDBC-3.0
   method 'getGeneratedKeys()' which is now the preferred method to use if you
   need to retrieve AUTO_INCREMENT keys and have access to JDBC-3.0. The second
   example shows how you can retrieve the same value using a standard 'SELECT
   LAST_INSERT_ID()' query. The final example shows how updatable result sets
   can retrieve the AUTO_INCREMENT value when using the method 'insertRow()'.
   ...
   that at times, it can be tricky to use the 'SELECT LAST_INSERT_ID()' query,
   as that function's value is scoped to a connection. So, if some other query
   happens on the same connection, the value will be overwritten. On the other
   hand, the 'getGeneratedKeys()' method is scoped by the Statement instance,
   so it can be used even if other queries happen on the same connection, but
...
1.2.1.1. Java Versions Supported
   ...
   turned off (-Xverify:none), as the class verifier will try to load the class
   definition for java.sql.Savepoint even though it is not accessed by the
   ...
   available on JVMs older than 1.4.x, as it relies on java.util.LinkedHashMap
...
1.2.1.2. MySQL Server Version Guidelines
   ...
   (foreign keys, updatable result sets) require more recent versions of MySQL
   to operate.
...
1.2.1.3. Installing the Driver and Configuring the CLASSPATH
   ...
   so when reporting a problem or bug to MySQL AB, as it is not designed to be
   run in production environments, and will have adverse performance impact
   when used. The debug binary also depends on the Aspect/J runtime library,
   which is located in the src/lib/aspectjrt.jar file that comes with the
   ...
   "tar" for the .tar.gz archive). Because there are potentially long filenames
   in the distribution, we use the GNU tar archive format. You will need to use
   GNU tar (or an application that understands the GNU tar archive format) to
   unpack the .tar.gz variant of the distribution.
   ...
   directly specifying it with the commandline switch -cp when starting your
   JVM
...
$ setenv CLASSPATH /path/to/mysql-connector-java-[version]-bin.jar:$CLASSPATH
   ...
   The above command can be added to the appropriate startup file for the login
   shell to make MySQL Connector/J available to all Java applications.

   If you want to use MySQL Connector/J with an application server such as
   Tomcat or JBoss, you will have to read your vendor's documentation for more
   information  on  how to configure third-party class libraries, as most
   application servers ignore the CLASSPATH environment variable. This document
   does contain configuration examples for some J2EE application servers in the
   section named "[2]Using Connector/J with J2EE and Other Java Frameworks",
   however the authoritative source for JDBC connection pool configuration
   information for your particular application server is the documentation for
   ...
   subdirectory of your webapp, as this is a standard location for third party
...
1.2.2. Upgrading from an Older Version
   ...
   versions to support new features, improve existing functionality, or comply
   with new standards.

   This section has information about what users who are upgrading from one
   version of Connector/J to another (or to a new version of the MySQL server,
   with respect to JDBC functionality) should be aware of.

1.2.2.1. Upgrading from MySQL Connector/J 3.0 to 3.1

   Connector/J 3.1 is designed to be backwards-compatible with Connector/J 3.0
   as much as possible. Major changes are isolated to new functionality exposed
   in MySQL-4.1 and newer, which includes Unicode character sets, server-side
   prepared statements, SQLState codes returned in error messages by the server
   and various performance enhancements that can be enabled or disabled via
   configuration properties.
     * Unicode Character Sets - See the next section, as well as the "Character
       Sets" section in the server manual for information on this new feature
       of MySQL. If you have something misconfigured, it will usually show up
       as an error with a message similar to 'Illegal mix of collations'.
     * Server-side Prepared Statements - Connector/J 3.1 will automatically
       ...
       Starting with version 3.1.7, the driver scans SQL you are preparing via
       all variants of Connection.prepareStatement() to determine if it is a
       supported type of statement to prepare on the server side, and if it is
       not supported by the server, it instead prepares it as a client-side
       emulated prepared statement. You can disable this feature by passing
       ...
       statement code that is still presently used for MySQL servers older than
       4.1.0 with the following connection property:
       useServerPrepStmts=false
     * Datetimes with all-zero components ('0000-00-00 ...') - These values can
       not be represented reliably in Java. Connector/J 3.0.x always converted
       them to NULL when being read from a ResultSet.
       ...
       zeroDateTimeBehavior ' configuration property. The allowable values are:
       'exception' (the default), which throws a SQLException with a SQLState
       of 'S1009', 'convertToNull', which returns NULL instead of the date, and
       'round', which rounds the date to the nearest closest value which is
       '0001-01-01'.
       Starting with Connector/J 3.1.7, ResultSet.getString() can be decoupled
       from this behavior via ' noDatetimeStringSync=true ' (the default value
       is 'false') so that you can get retrieve the unaltered all-zero value as
       a String. It should be noted that this also precludes using any timezone
       conversions,  therefore  the  driver  will not allow you to enable
     ...
     * New  SQLState Codes - Connector/J 3.1 uses SQL:1999 SQLState codes
       ...
       the "legacy" X/Open state codes that Connector/J 3.0 uses. If connected
       to a MySQL server older than MySQL-4.1.0 (the oldest version to return
       SQLStates as part of the error code), the driver will use a built-in
       mapping.  You can revert to the old mapping by using the following
       configuration property:
       useSqlStateCodes=false
     * Calling ResultSet.getString() on a BLOB column will now return the
       ...
       To store strings in MySQL with LOB behavior, use one of the TEXT types,
       which the driver will treat as a java.sql.Clob.
     * Starting with Connector/J 3.1.8 a "debug" build of the driver in a file
       named "mysql-connector-java-[version]-bin-g.jar" is shipped alongside
...
for Connector/J
   ...
   is 'com.mysql.jdbc.Driver'. The 'org.gjt.mm.mysql.Driver' class name is also
   usable to remain backwards-compatible with MM.MySQL. You should use this
   class  name when registering the driver, or when otherwise configuring
...
jdbc:mysql://[host][,failoverhost...][:port]/[database][?propertyName1][=proper
tyValue1][&propertyName2][=propertyValue2]...
...
ue1][&propertyName2][=propertyValue2]...
   ...
   MySQL Connector/J has fail-over support. This allows the driver to fail-over
   to  any  number  of "slave" hosts and still perform read-only queries.
   ...
   Most application servers and connection pools set autoCommit to 'true' at
   the end of every transaction/connection use.
   ...
   connection will be set to read-only state, so queries that would modify data
   will have exceptions thrown (the query will never be processed by the MySQL
   server).

   Configuration properties define how Connector/J will make a connection to a
   MySQL server. Unless otherwise noted, properties can be set for a DataSource
   object or for a Connection object.
   ...
   Configuration Properties can be set in one of the following ways:
     ...
     * As    a    JDBC    URL    parameter    in   the   URL   given   to
       java.sql.DriverManager.getConnection(),java.sql.Driver.connect()orthe
       MySQL implementations of javax.sql.DataSource's setURL() method.
...
Note
   ...
   *pattern parameters treat null the same as '%' (this is not JDBC-compliant,
   ...
   because UltraDev is broken and issues a prepareCall() for _all_ statements?
   ...
   the 'socketFactory' property. If you don't use a 'namedPipePath' property,
   the  default  of  '\\.\pipe\MySQL'  will  be  used.  If  you  use  the
   NamedPipeSocketFactory, the hostname and port number values in the JDBC url
   will be ignored.
   ...
   Named pipes only work when connecting to a MySQL server on the same physical
   machine as the one the JDBC driver is being used on. In simple performance
   tests, it appears that named pipe access is between 30%-50% faster than the
   standard TCP/IP access.
...
1.3.2. JDBC API Implementation Notes
   ...
   MySQL Connector/J passes all of the tests in the publicly-available version
   of  Sun's  JDBC compliance testsuite. However, in many places the JDBC
   specification  is  vague  about  how  certain  functionality should be
   ...
   certain implementation decisions may affect how you use MySQL Connector/J.
     * Blob
       ...
       locators by adding the property 'emulateLocators=true' to your JDBC URL.
       You must then use a column alias with the value of the column set to the
       actual name of the Blob column in the SELECT that you write to retrieve
       the Blob. The SELECT must also reference only one table, the table must
       have a primary key, and the SELECT must cover all columns that make up
       the primary key. The driver will then delay loading the actual Blob data
       until   you   retrieve   the   Blob  and  call  retrieval  methods
       (getInputStream(), getBytes(), etc) on it.
     * CallableStatement
     ...
     * Clob
       ...
       'copies', as reported by the DatabaseMetaData.locatorsUpdateCopies()
       method). Because of this, you should use the PreparedStatement.setClob()
       method to save changes back to the database. The JDBC API does not have
     ...
     * DatabaseMetaData
       ...
       information, so when other table types support foreign keys, the driver
       will transparently support them as well.
     * Driver
     * PreparedStatement
       ...
       parameters again. The reason for this is as follows:
          + The driver streams the 'large' data 'out-of-band' to the prepared
            statement on the server side when the parameter is set (before
            execution of the prepared statement).
          + Once that has been done, the stream used to read the data on the
            client side is closed (as per the JDBC spec), and can't be read
            from again.
          + If a parameter changes from "large" to non-"large", the driver must
            reset the server-side state of the prepared statement to allow the
            parameter that is being changed to take the place of the prior
            "large"  value. This removes all of the 'large' data that has
            already been sent to the server, thus requiring the data to be
       ...
       non-"large" one, you must call clearParameters() and set all parameters
...
stmt.setFetchSize(Integer.MIN_VALUE);
       ...
       queries on the connection, or an exception will be thrown.
       ...
       statement are read or the active result set for the statement is closed.
     ...
     * ResultSetMetaData
       ...
       and newer.
     * Statement
       When using versions of the JDBC driver earlier than 3.2.1, and connected
       to server versions earlier than 5.0.3, the "setFetchSize()" method has
       no effect, other than to toggle result set streaming as described above.
       MySQL does not support SQL cursors, and the JDBC driver doesn't emulate
...
1.3.3. Java, JDBC and MySQL Types
   ...
   In general, any MySQL data type can be converted to a java.lang.String, and
   any numerical type can be converted to any of the Java numerical types,
...
Note
   ...
   The ResultSet.getObject() method uses the following type conversions between
   MySQL and Java types, following the JDBC specification where appropriate:
...
1.3.4. Using Character Sets and Unicode
   ...
   Prior to MySQL Server 4.1, Connector/J supported a single character encoding
   per connection, which could either be automatically detected from the server
   configuration, or could be configured by the user through the useUnicode and
   ...
   upon connection. The encoding used by the driver is specified on the server
   via the configuration variable ' character_set ' for server versions older
   than 4.1.0 and ' character_set_server ' for server versions 4.1.0 and newer.
   See the "Server Character Set and Collation
   (http://www.mysql.com/doc/en/Charset-server.html)" section in the MySQL
   server manual for more information.

   To override the automatically-detected encoding on the client side, use the
   characterEncoding property in the URL used to connect to the server.
...
1.3.5. Connecting Securely Using SSL
   ...
   50%, depending on the size of the query, and the amount of data it returns.

   For SSL Support to work, you must have the following:
     * A JDK that includes JSSE (Java Secure Sockets Extension), like JDK-1.4.1
       or newer. SSL does not currently work with a JDK that you can add JSSE
       to,  like  JDK-1.2.x  or  JDK-1.3.x due to the following JSSE bug:
       http://developer.java.sun.com/developer/bugParade/bugs/4273544.html
     * A MySQL server that supports SSL and has been compiled and configured to
       do so, which is MySQL-4.0.4 or later, see:
   ...
   subdirectory of the MySQL source distribution. This is what SSL will use to
...
Certificate was added to keystore
 ...
 shell> keytool -genkey -keyalg rsa -alias mysqlClientCertificate -keystore key
store

   Keytool will prompt you for the following information, and create a keystore
   named 'keystore' in the current directory.
...
-Djavax.net.ssl.trustStorePassword=*********
   ...
   setting the property 'useSSL' to 'true' in the java.util.Properties instance
   you pass to DriverManager.getConnection().
...
1.3.6. Using Master/Slave Replication with ReplicationConnection
   ...
   will use one of the slave connections, which are load-balanced per-vm using
   a round-robin scheme (a given connection is "sticky" to a slave unless that
   slave is removed from service). If you have a write transaction, or if you
   have a read that is "time-sensitive" (remember, replication in MySQL is
   asynchronous),  set  the  connection  to  be not read-only, by calling
   Connection.setReadOnly(false) and the driver will ensure that further calls
   are sent to the "master" MySQL server. The driver takes care of propagating
   the current state of autocommit, isolation level, and catalog between all of
   ...
   ReplicationDriver does not currently work with java.sql.DriverManager -based
   connection creation unless it is the only MySQL JDBC driver registered with
   the DriverManager .
...
1.4.1. General J2EE Concepts
   ...
   This section provides general background on J2EE concepts that pertain to
   use of Connector/J.
...
1.4.1.1. Understanding Connection Pooling
   ...
   milliseconds to complete. When not processing a transaction, the connection
   would  otherwise sit idle. Instead, connection pooling allows the idle
   ...
   database with JDBC, it requests a connection from the pool. When the thread
   is finished using the connection, it returns it to the pool, so that it may
   be used by any other threads that want to use it.

   When the connection is "loaned out" from the pool, it is used exclusively by
   the thread that requested it. From a programming point of view, it is the
   same as if your thread called DriverManager.getConnection() every time it
   needed a JDBC connection, however with connection pooling, your thread may
   end up using either a new, or already-existing connection.
   ...
   connection pooling are:
     * Reduced connection creation time
       ...
       MySQL offers compared to other databases, creating new JDBC connections
       still incurs networking and JDBC driver overhead that will be avoided if
       connections are "recycled."
     * Simplified programming model
       When using connection pooling, each individual thread can act as though
       it  has  created  its  own  JDBC  connection,  allowing you to use
       straight-forward JDBC programming techniques.
     * Controlled resource usage
       If you don't use connection pooling, and instead create a new connection
       every time a thread needs one, your application's resource usage can be
       quite wasteful and lead to unpredictable behavior under load.
...
public class MyServletJspOrEjb {
   ...
   As shown in the example above, after obtaining the JNDI InitialContext, and
   looking up the DataSource, the rest of the code should look familiar to
   anyone who has done JDBC programming in the past.
   ...
   MySQL server resources they represent (buffers, locks, sockets, etc) may be
   tied up for some time, or worst case, may be tied up forever.

   What's the Best Size for my Connection Pool?

   As with all other configuration rules-of-thumb, the answer is "It depends."
   While the optimal size depends on anticipated load and average database
   transaction time, the optimum connection pool size is smaller than you might
   expect. If you take Sun's Java Petstore blueprint application for example, a
   connection pool of 15-20 connections can serve a relatively moderate load
   (600 concurrent users) using MySQL and Tomcat with response times that are
   acceptable.

   To correctly size a connection pool for your application, you should create
   load test scripts with tools such as Apache JMeter or The Grinder, and load
   test your application.

   An easy way to determine a starting point is to configure your connection
   pool's maximum number of connections to be "unbounded," run a load test, and
   measure the largest amount of concurrently used connections. You can then
   work backwards from there to determine what values of minimum and maximum
   pooled  connections  give  the  best  performance  for your particular
   application.
...
<Context ....>
  ...
  <ResourceParams name="jdbc/MySQLDB">
   ...
   changes from time-to-time, and unfortunately if you use the wrong syntax in
   your XML file, you will most likely end up with an exception similar to the
   following:
...
1.4.3. Using Connector/J with JBoss
   ...
   usually called "default"). Then, in the same configuration directory, in the
   subdirectory named "deploy", create a datasource configuration file that
   ends  with  "-ds.xml", which tells JBoss to deploy this file as a JDBC
...
1.5. Diagnosing Connector/J Problems
   ...
   This section describes how to solve problems that you may encounter when
   using Connector/J.
...
Warning
   ...
   section of the MySQL manual before attempting to change privileges.
   ...
   My application throws a SQLException 'No Suitable Driver'. Why is this
   happening?
...
1.5.2. How to Report Bugs or Problems
   ...
   security@mysql.com (mailto:security@mysql.com).
   ...
   A good principle is this: If you are in doubt about stating something, state
   ...
   number  of Connector/J or MySQL used, and (b) not fully describing the
   platform on which Connector/J is installed (including the JVM version, and
   the platform type and version number that MySQL itself is installed on).
   ...
   If at all possible, you should create a repeatable, stanalone testcase that
   ...
   getConnection () method to create a JDBC connection to MySQL:
     ...
     * getNewConnection() - Use this if you need to get a new connection for
     ...
     * getConnection(String url, Properties props) - Returns a connection using
       the given URL and properties.
   ...
   boolean expression) methods to create conditions that must be met in your
...
public static void main(String[] args) throws Exception {
   ...
   Once you have finished your testcase, and have verified that it demonstrates
   the  bug  you  are  reporting,  upload  it  with  your  bug  report to
   http://bugs.mysql.com/.
...
s
      and MySQL 5.0 or newer. Result set needs to be forward-only, and a non-ze
ro
...
esn't work when using
    ...
    - Fixed BUG#11798 (http://bugs.mysql.com/11798) - Pstmt.setObject(...., Typ
es.BOOLEAN) throws exception.
...
ent in
      DatabaseMetaData.getIndexInfo(), which also means changes to the followin
g
    ...
    - getTables()
      ...
      The "catalog" argument in all of these methods now behaves in the followi
ng
      way:
...
rned for
    ...
    - Fixed BUG#13277 (http://bugs.mysql.com/13277) - ResultSetMetaData from
...
ers don't allow
    ...
    - Fixed BUG#13043 (http://bugs.mysql.com/13043) - when 'gatherPerfMetrics'
is enabled for
...
06-22-05 - Version 3.1.9-stable
    ...
    - Fixed BUG#9064 (http://bugs.mysql.com/9064) - com.mysql.jdbc.PreparedStat
ement.ParseInfo
      does unnecessary call to toCharArray().

    - Fixed Bug#10144 (http://bugs.mysql.com/10144) - Memory leak in ServerPrep
aredStatement if
      serverPrepare() fails.
    ...
    - Fixed BUG#10156 (http://bugs.mysql.com/10156) - Unsigned SMALLINT treated
 as signed for ResultSet.getInt(),
      fixed all cases for UNSIGNED integer values and server-side prepared stat
ements,
      as well as ResultSet.getObject() for UNSIGNED TINYINT.

    - Fixed BUG#10155 (http://bugs.mysql.com/10155), double quotes not recogniz
ed when parsing
    ...
    - Fixed BUG#10310 (http://bugs.mysql.com/10310) - Driver doesn't support {?
=CALL(...)} for calling
    ...
    - Fixed BUG#10485 (http://bugs.mysql.com/10485), SQLException thrown when r
etrieving YEAR(2)
    ...
    - The datatype returned for TINYINT(1) columns when "tinyInt1isBit=true"
      ...
      Types.BOOLEAN will be returned instead. Irregardless of this configuratio
n
    ...
    - Fixed BUG#10496 (http://bugs.mysql.com/10496) - SQLException is thrown wh
en using property
    ...
    - Fixed BUG#10850 (http://bugs.mysql.com/10850) - 0-length streams not sent
 to server when
...
hen using server-side
    ...
    - Added "maintainTimeStats" configuration property (defaults to "true"),
      which tells the driver whether or not to keep track of the last query tim
e
      and the last successful packet sent to the server's time. If set to
      false, removes two syscalls per query.

    - Fixed BUG#11259 (http://bugs.mysql.com/11259), autoReconnect ping causes
exception on connection
      startup.

    - Fixed BUG#11360 (http://bugs.mysql.com/11360) Connector/J dumping query i
nto SQLException twice

    - Fixed PreparedStatement.setClob() not accepting null as a parameter.

    - Fixed BUG#11411 (http://bugs.mysql.com/11411) - Production package doesn'
t include JBoss integration
      classes.
...
04-14-05 - Version 3.1.8-stable
    ...
    - Fixed Bug#8868 (http://bugs.mysql.com/8868), DATE_FORMAT() queries return
ed as BLOBs from getObject().
    ...
    - Fixed BUG#9040 (http://bugs.mysql.com/9040) - PreparedStatement.addBatch(
) doesn't work with server-side
      prepared statements and streaming BINARY data.

    - Fixed BUG#8800 (http://bugs.mysql.com/8800) - DBMD.supportsMixedCase*Iden
tifiers() returns wrong
      value on servers running on case-sensitive filesystems.

    - Fixed BUG#9206 (http://bugs.mysql.com/9206), can not use 'UTF-8' for char
acterSetResults
      configuration property.

    - Fixed BUG#9236 (http://bugs.mysql.com/9236), a continuation of BUG#8868 (
http://bugs.mysql.com/8868), where functions used in queries
      that should return non-string types when resolved by temporary tables sud
denly
      become opaque binary strings (work-around for server limitation). Also fi
xed
      fields with type of CHAR(n) CHARACTER SET BINARY to return correct/matchi
ng
      classes for RSMD.getColumnClassName() and ResultSet.getObject().

    - Fixed BUG#8792 (http://bugs.mysql.com/8792) - DBMD.supportsResultSetConcu
rrency() not returning
      true for forward-only/read-only result sets (we obviously support this).

    - Fixed BUG#8803 (http://bugs.mysql.com/8803), 'DATA_TYPE' column from DBMD
.getBestRowIdentifier()
      causes ArrayIndexOutOfBoundsException when accessed (and in fact, didn't
      return any value).

    - Check for empty strings ('') when converting char/varchar column data to
numbers,
      throw exception if 'emptyStringsConvertToZero' configuration property is
set
    ...
    - Fixed BUG#9320 (http://bugs.mysql.com/9320) - PreparedStatement.getMetaDa
ta() inserts blank row in database
      under certain conditions when not using server-side prepared statements.

    - Connection.canHandleAsPreparedStatement() now makes 'best effort' to dist
inguish
      LIMIT clauses with placeholders in them from ones without in order to hav
e fewer
      false positives when generating work-arounds for statements the server ca
nnot
    ...
    - Fixed BUG#9319 (http://bugs.mysql.com/9319) - Stored procedures with same
 name in
    ...
    - Fixed BUG#9682 (http://bugs.mysql.com/9682) - Stored procedures with DECI
MAL parameters with
    ...
    - Fixed BUG#9704 (http://bugs.mysql.com/9704) - Statement.getMoreResults()
could throw NPE when
    ...
    - Fixed BUG#9778 (http://bugs.mysql.com/9778), DBMD.getTables() shouldn't r
eturn tables if views
    ...
    - Fixed BUG#9917 (http://bugs.mysql.com/9917) - Should accept null for cata
log (meaning use current)
      in DBMD methods, even though it's not JDBC-compliant for legacy's sake.
      Disable by setting connection property "nullCatalogMeansCurrent" to "fals
e"
      (which will be the default value in C/J 3.2.x).

    - Fixed BUG#9769 (http://bugs.mysql.com/9769) - Should accept null for name
 patterns in DBMD (meaning "%"),
      even though it isn't JDBC compliant, for legacy's sake. Disable by settin
g
...
02-18-05 - Version 3.1.7-stable
    ...
    - Fixed BUG#7686 (http://bugs.mysql.com/7686), Timestamp key column data ne
eded "_binary'"
      stripped for UpdatableResultSet.refreshRow().

    - Fixed BUG#7715 (http://bugs.mysql.com/7715) - Timestamps converted incorr
ectly to strings
    ...
    - Fixed BUG#7952 (http://bugs.mysql.com/7952) -- Infinite recursion when 'f
alling back' to master
    ...
    - Fixed BUG#8096 (http://bugs.mysql.com/8096) where emulated locators corru
pt binary data
    ...
    - By default, the driver now scans SQL you are preparing via all
      ...
      client-side emulated prepared statement (BUG#4718 (http://bugs.mysql.com/
4718)). You can
    ...
    - Fixed BUG#8428 (http://bugs.mysql.com/8428) - ResultSet.getString() doesn
't maintain format
    ...
    - Fixed BUG#8487 (http://bugs.mysql.com/8487) - PreparedStatements not crea
ting streaming result
      sets.
    ...
    - Fixed BUG#8484 (http://bugs.mysql.com/8484) - ResultSet.getBigDecimal() t
hrows exception
...
12-23-04 - Version 3.1.6-stable
    ...
    - Fixed BUG#7026 (http://bugs.mysql.com/7026) - DBMD.getProcedures() doesn'
t respect catalog parameter.
...
12-02-04 - Version 3.1.5-gamma
    ...
    - Fixed BUG#5235 (http://bugs.mysql.com/5235) - Server-side prepared statem
ents did not honor
    ...
    - Fixed BUG#5706 (http://bugs.mysql.com/5706) - Inefficient detection of pr
e-existing string instances
    ...
    - Fixed BUG#5729 (http://bugs.mysql.com/5729) - UNSIGNED BIGINT unpacked in
correctly from
      server-side prepared statement result sets.

    - Fixed BUG#6225 (http://bugs.mysql.com/6225) - ServerSidePreparedStatement
 allocating short-lived
    ...
    - Removed un-wanted new Throwable() in ResultSet constructor due to bad
      ...
      set created) - Found while profiling for BUG#6359 (http://bugs.mysql.com/
6359).
    ...
    - Fixed too-early creation of StringBuffer in EscapeProcessor.escapeSQL(),
      ...
      allocations). Found while profiling for BUG#6359 (http://bugs.mysql.com/6
359).

    - Use null-safe-equals for key comparisons in updatable result sets.

    - Fixed BUG#6537 (http://bugs.mysql.com/6537), SUM() on Decimal with server
-side prepared statement ignores
      scale if zero-padding is needed (this ends up being due to conversion to
DOUBLE
      by server, which when converted to a string to parse into BigDecimal, los
es all
      'padding' zeros).
    ...
    - Fixed BUG#6399 (http://bugs.mysql.com/6399), ResultSetMetaData.getColumnD
isplaySize() returns incorrect
    ...
    - Fixed BUG#6348 (http://bugs.mysql.com/6348), failing to connect to the se
rver when one of the
...
09-04-04 - Version 3.1.4-beta
    ...
    - Fixed BUG#4510 (http://bugs.mysql.com/4510) - connector/j 3.1.3 beta does
 not handle integers
    ...
    - Fixed BUG#4642 (http://bugs.mysql.com/4642) -- ServerPreparedStatement.ex
ecute*() sometimes
...
.
    ...
    - Fixed BUG#4689 (http://bugs.mysql.com/4689) - ResultSet.wasNull() does no
t work for primatives if a
      previous null was returned.

    - Track packet sequence numbers if enablePacketDebug=true, and throw an
      exception if packets received out-of-order.

    - Fixed BUG#4482 (http://bugs.mysql.com/4482), ResultSet.getObject() return
s wrong type for strings
      when using prepared statements.

    - Calling MysqlPooledConnection.close() twice (even though an application
      error), caused NPE. Fixed.

    - Fixed BUG#5012 (http://bugs.mysql.com/5012) -- ServerPreparedStatements d
ealing with return of
      DECIMAL type don't work.

    - Fixed BUG#5032 (http://bugs.mysql.com/5032) -- ResultSet.getObject() does
n't return
    ...
    - Added three ways to deal with all-zero datetimes when reading them from
      ...
      with a SQLState of 'S1009', 'convertToNull', which returns NULL instead o
f
...
07-07-04 - Version 3.1.3-beta
    ...
    - Fix for BUG#4119 (http://bugs.mysql.com/4119), null bitmask sent for serv
er-side prepared
    ...
    - Fix for BUG#4311 (http://bugs.mysql.com/4311) - Error in retrieval of med
iumint column with
    ...
    - Support for unsigned numerics as return types from prepared statements.
      This also causes a change in ResultSet.getObject() for the 'bigint unsign
ed'
...
06-09-04 - Version 3.1.2-alpha
    ...
    - Correctly map output parameters to position given in
      ...
      fixes BUG#3146 (http://bugs.mysql.com/3146).
    ...
    - Fix for BUG#3539 (http://bugs.mysql.com/3539) getProcedures() does not re
turn any procedures in
      result set

    - Fix for BUG#3540 (http://bugs.mysql.com/3540) getProcedureColumns() doesn
't work with wildcards
      for procedure name

    - Fixed BUG#3520 (http://bugs.mysql.com/3520) -- DBMD.getSQLStateType() ret
urns incorrect value.
    ...
    - Fixed BUG#3804 (http://bugs.mysql.com/3804) -- getWarnings() returns SQLW
arning instead of DataTruncation

    - Don't enable server-side prepared statements for server version 5.0.0 or
5.0.1,
    as they aren't compatible with the '4.1.2+' style that the driver uses (the
 driver
...
02-14-04 - Version 3.1.1-alpha
    ...
    - Fix for BUG#1673 (http://bugs.mysql.com/1673), where DatabaseMetaData.get
Columns() is not
    ...
    - Added named and indexed input/output parameter support to CallableStateme
nt.
    ...
    - Fixed BUG#2359 (http://bugs.mysql.com/2359), NULL values for numeric type
s in binary
    ...
    - Fixed BUG#2502 (http://bugs.mysql.com/2502) -- charset conversion issue i
n getTables().

    - Implemented multiple result sets returned from a statement
      or stored procedure.

    - Fixed BUG#2606 (http://bugs.mysql.com/2606) -- Server side prepared state
ments not returning
    ...
    - Fixed BUG#2623 (http://bugs.mysql.com/2623) -- Class-cast exception when
using
    ...
    - Fixed BUG#2671 (http://bugs.mysql.com/2671), NULL fields not being encode
d correctly in
...
06-23-05 - Version 3.0.17-ga
    ...
    - Fixed BUG#5874 (http://bugs.mysql.com/5874), Timestamp/Time conversion go
es in the wrong 'direction'
      when useTimeZone='true' and server timezone differs from client timezone.

    - Fixed BUG#7081 (http://bugs.mysql.com/7081), DatabaseMetaData.getIndexInf
o() ignoring 'unique'
      parameter.
    ...
    - Fixed BUG#7316 (http://bugs.mysql.com/7316) - Statements created from a p
ooled connection were
      returning physical connection instead of logical connection when
      getConnection() was called.

    - Fixed BUG#7033 (http://bugs.mysql.com/7033) - PreparedStatements don't en
code Big5 (and other
      multi-byte) character sets correctly in static SQL strings.

    - Fixed BUG#6966 (http://bugs.mysql.com/6966), connections starting up fail
ed-over (due to down master)
      never retry master.

    - Fixed BUG#7061 (http://bugs.mysql.com/7061), PreparedStatement.fixDecimal
Exponent() adding extra
      '+', making number unparseable by MySQL server.

    - Fixed BUG#7686 (http://bugs.mysql.com/7686), Timestamp key column data ne
eded "_binary'" stripped for
    ...
    - Fixed BUG#7601 (http://bugs.mysql.com/7601), PreparedStatement.fixDecimal
Exponent() adding extra
      '+', making number unparseable by MySQL server.
    ...
    - Fixed BUG#7607 (http://bugs.mysql.com/7607) - MS932, SHIFT_JIS and Window
s_31J not recog. as
      aliases for sjis.

    - Fixed BUG#6549 (http://bugs.mysql.com/6549) (while fixing #7607), adding
'CP943' to aliases for
      sjis.

    - Fixed BUG#8064 (http://bugs.mysql.com/8064), which requires hex escaping
of binary data when using
      multi-byte charsets with prepared statements.

    - Fixed BUG#8812 (http://bugs.mysql.com/8812), NON_UNIQUE column from DBMD.
getIndexInfo() returned
      inverted value.

    - Workaround for server BUG#9098 (http://bugs.mysql.com/9098) - default val
ues of CURRENT_* for
    ...
    - Fixed BUG#8629 (http://bugs.mysql.com/8629) - 'EUCKR' charset is sent as
'SET NAMES euc_kr' which
...
11-15-04 - Version 3.0.16-ga
    ...
    - Fixed BUG#5664 (http://bugs.mysql.com/5664), ResultSet.updateByte() when
on insert row
    ...
    - Fixed BUG#6198 (http://bugs.mysql.com/6198), off-by-one bug in Buffer.rea
dString(string).
...
'
    ...
    - Fixed BUG#6231 (http://bugs.mysql.com/6231), ResultSet.getTimestamp() on
a column with TIME in it
      fails.

09-04-04 - Version 3.0.15-production

    - Fixed BUG#4010 (http://bugs.mysql.com/4010) - StringUtils.escapeEasternUn
icodeByteStream is still
      broken for GBK

    - Fixed BUG#4334 (http://bugs.mysql.com/4334) - Failover for autoReconnect
not using port #'s for any
    ...
    - Fixed BUG#4742 (http://bugs.mysql.com/4742), 'DOUBLE' mapped twice in DBM
D.getTypeInfo().

    - Added FLOSS license exemption.

    - Fixed BUG#4808 (http://bugs.mysql.com/4808), calling .close() twice on a
PooledConnection causes NPE.

    - Fixed BUG#4138 (http://bugs.mysql.com/4138) and BUG#4860 (http://bugs.mys
ql.com/4860), DBMD.getColumns() returns incorrect JDBC
    ...
    - Fix for BUG#4880 (http://bugs.mysql.com/4880), RSMD.getPrecision() return
ing 0 for non-numeric types
      ...
      RSMD.getColumnType() and RSMD.getColumnTypeName() for the BLOB types base
d
      on the length sent from the server (the server doesn't distinguish betwee
n
      TINYBLOB, BLOB, MEDIUMBLOB or LONGBLOB at the network protocol level).

    - Fixed BUG#5022 (http://bugs.mysql.com/5022) - ResultSet should release Fi
eld[] instance in .close().

    - Fixed BUG#5069 (http://bugs.mysql.com/5069) -- ResultSet.getMetaData() sh
ould not return
...
05-27-04 - Version 3.0.13-production
    ...
    - Fixed BUG#3848 (http://bugs.mysql.com/3848) - Using a MySQLDatasource wit
hout server name fails

    - Fixed BUG#3920 (http://bugs.mysql.com/3920) - "No Database Selected" when
 using
    MysqlConnectionPoolDataSource.

    - Fixed BUG#3873 (http://bugs.mysql.com/3873) - PreparedStatement.getGenera
tedKeys() method returns only
...
05-18-04 - Version 3.0.12-production
    ...
    - Allow java.util.Date to be sent in as parameter to
      ...
      to maintain full precision (BUG#3103 (http://bugs.mysql.com/3103)).

    - Don't truncate BLOBs/CLOBs when using setBytes() and/or
      setBinary/CharacterStream() (BUG#2670 (http://bugs.mysql.com/2670)).
    ...
    - Fixed BUG#3511 (http://bugs.mysql.com/3511), StringUtils.escapeSJISByteSt
ream() not covering all
      eastern double-byte charsets correctly.

    - Renamed StringUtils.escapeSJISByteStream() to more appropriate
      escapeEasternUnicodeByteStream().

    - Fixed BUG#3554 (http://bugs.mysql.com/3554) - Not specifying database in
URL caused MalformedURL
      exception.
    ...
    - Fixed BUG#3557 (http://bugs.mysql.com/3557) - UpdatableResultSet not pick
ing up default values
      for moveToInsertRow().

    - Fixed BUG#3570 (http://bugs.mysql.com/3570) - inconsistent reporting of c
olumn type. The server
...
02-19-04 - Version 3.0.11-stable
    ...
    - AutoReconnect time was growing faster than exponentially (BUG#2447 (http:
//bugs.mysql.com/2447)).

    - Fixed failover always going to last host in list (BUG#2578 (http://bugs.m
ysql.com/2578))
    ...
    - Return 'java.lang.Integer' for TINYINT and SMALLINT types from
      ResultSetMetaData.getColumnClassName() (fix for BUG#2852 (http://bugs.mys
ql.com/2852)).

    - Return 'java.lang.Double' for FLOAT type from ResultSetMetaData.
      getColumnClassName() (fix for BUG#2855 (http://bugs.mysql.com/2855)).
...
01-13-04 - Version 3.0.10-stable
    ...
    - Don't count quoted id's when inside a 'string' in PreparedStatement
      parsing (fix for BUG#1511 (http://bugs.mysql.com/1511)).
    ...
    - Fixed BUG#1576 (http://bugs.mysql.com/1576), dealing with large blobs and
 split packets not being
      read correctly.

    - Fixed regression of Statement.getGeneratedKeys() and REPLACE statements.

    - Fixed BUG#1630 (http://bugs.mysql.com/1630), subsequent call to ResultSet
.updateFoo() causes NPE if
      result set is not updatable.

    - Fix for 4.1.1-style auth with no password.

    - Fix for BUG#1731 (http://bugs.mysql.com/1731), Foreign Keys column sequen
ce is not consistent in
      DatabaseMetaData.getImported/Exported/CrossReference().

    - Fix for BUG#1775 (http://bugs.mysql.com/1775) - DatabaseMetaData.getSyste
mFunction() returning
      bad function 'VResultsSion'.

    - Fix for BUG#1592 (http://bugs.mysql.com/1592) -- cross-database updatable
 result sets
    ...
    - ResultSet.getObject() on TINYINT and SMALLINT columns should return
      Java type 'Integer' (BUG#1913 (http://bugs.mysql.com/1913))
    ...
    - Fix for BUG#1933 (http://bugs.mysql.com/1933) -- Connection property "max
Rows" not honored.

    - Fix for BUG#1925 (http://bugs.mysql.com/1925) -- Statements being created
 too many times in
      DBMD.extractForeignKeyFromCreateTable().

    - Fix for BUG#1914 (http://bugs.mysql.com/1914) -- Support escape sequence
{fn convert ... }

    - Fix for BUG#1958 (http://bugs.mysql.com/1958) -- ArrayIndexOutOfBounds wh
en parameter number ==
      number of parameters + 1.

    - Fix for BUG#2006 (http://bugs.mysql.com/2006) -- ResultSet.findColumn() s
hould use first matching
...
10-07-03 - Version 3.0.9-stable
    ...
    - Fixed ResultSet.previous() behavior to move current
      ...
      of result set (bugs.mysql.com BUG#496 (http://bugs.mysql.com/496))

    - Fixed Statement and PreparedStatement issuing bogus queries
      when setMaxRows() had been used and a LIMIT clause was present
      in the query.

    - Fixed BUG#661 (http://bugs.mysql.com/661) - refreshRow didn't work when p
rimary key values
    ...
    - Support InnoDB contraint names when extracting foreign key info
      in DatabaseMetaData BUG#517 (http://bugs.mysql.com/517) and BUG#664 (http
://bugs.mysql.com/664)
    ...
    - Don't change timestamp TZ twice if useTimezone==true (BUG#774 (http://bug
s.mysql.com/774))

    - Fixed regression in large split-packet handling (BUG#848 (http://bugs.mys
ql.com/848)).
    ...
    - Don't fire connection closed events when closing pooled connections, or
      on PooledConnection.getConnection() with already open connections (BUG#88
4 (http://bugs.mysql.com/884)).

    - Clip +/- INF (to smallest and largest representative values for the type
in
      MySQL) and NaN (to 0) for setDouble/setFloat(), and issue a warning on th
e
      statement when the server does not support +/- INF or NaN.

    - Fix for BUG#879 (http://bugs.mysql.com/879), double-escaping of '\' when
charset is SJIS or GBK and '\'
    ...
    - Fixed BUG#1099 (http://bugs.mysql.com/1099), DatabaseMetaData.getColumns(
    ...
    - Optimized CLOB.setChracterStream(), BUG#1131 (http://bugs.mysql.com/1131)

    - Made databaseName, portNumber and serverName optional parameters
      for MysqlDataSourceFactory (BUG#1246 (http://bugs.mysql.com/1246))

    - Fix for BUG#1247 (http://bugs.mysql.com/1247) -- ResultSet.get/setString
mashing char 127
...
05-23-03 - Version 3.0.8-stable
    ...
    - Unicode character 0xFFFF in a string would cause the driver to
      throw an ArrayOutOfBoundsException (Bug #378 (http://bugs.mysql.com/378))
...
09-21-02 - Version 3.0.1-dev
    ...
    - Massive code clean-up to follow Java coding conventions (the time had com
e)
...
,
    ...
    - ResultSet.getDouble() now uses code built into JDK to be more precise (bu
t slower)
...
References
   ...
   1. file://localhost/home/mysqldoc/docs-build/mysqldoc/refman-5.0/connector-j-nolink.html#cj-installing
   2. file://localhost/home/mysqldoc/docs-build/mysqldoc/refman-5.0/connector-j-nolink.html#cj-j2ee
